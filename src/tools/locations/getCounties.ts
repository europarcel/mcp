...
export function registerGetCountiesTool(server: McpServer): void {
  ...
  server.registerTool(
    ...
    {
      title: "Get Counties",
      description: "Retrieves counties for a specific country. Requires country_code parameter.",
      inputSchema: {
        country_code: z.string().describe("The country code (e.g., 'RO' for Romania, 'BG' for Bulgaria)")
      }
    },
    async (args: any) => {
