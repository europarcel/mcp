...
export function registerTrackAwbsByCarrierTool(server: McpServer): void {
  ...
  server.registerTool(
    "trackAwbsByCarrier",
    {
      ...
      description: "Track multiple AWB numbers from a specific carrier. Parameters: carrier_id (required - accepts numbers like 3 and strings like '3'), awb_list (array, max 200, required), language (optional, default 'ro')",
      inputSchema: {
        carrier_id: z.union([z.string(), z.number()]).describe("The carrier ID to track AWBs from"),
        awb_list: z.array(z.string()).max(200).describe("Array of AWB numbers to track (maximum 200)"),
    ...
    async (args: any) => {
      ...
      try {
        ...
        if (!args.awb_list || !Array.isArray(args.awb_list) || args.awb_list.length === 0) {
          return {
            content: [
              {
                type: "text",
        ...
        const trackingInfo = await client.trackAwbsByCarrier(carrierId, args.awb_list, language);
        ...
        if (trackingInfo.length === 0) {
          formattedResponse += "No tracking information found for the provided AWBs.";
        } else {
