...
export function registerGetOrdersTool(server: McpServer): void {
  ...
  server.registerTool(
    "getOrders",
    {
      ...
      description: "Get list of customer orders with tracking information. Parameters: page (optional), per_page (15-200, optional)",
      inputSchema: {
        page: z.number().int().positive().optional().describe("Page number for pagination (default: 1)"),
        per_page: z.number().int().min(15).max(200).optional().describe("Number of orders per page (15-200, default: 15)")
      }
    },
    async (args: any) => {
