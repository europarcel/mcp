...
export function registerCancelOrderTool(server: McpServer): void {
  ...
  server.registerTool(
    ...
    {
      title: "Cancel Order",
      description: "Cancel an existing order. Parameters: order_id (required - accepts numbers like 6505 and strings like '6505'), refund_channel ('wallet' or 'card', required)",
      inputSchema: {
        order_id: z.union([z.string(), z.number()]).describe("The order ID to cancel"),
        refund_channel: z.enum(["wallet", "card"]).describe("Where to process the refund: 'wallet' or 'card'")
      }
    },
    async (args: any) => {
      ...
      try {
        ...
        if (!args.refund_channel || !['wallet', 'card'].includes(args.refund_channel)) {
          return {
            content: [
              {
                type: "text",
        ...
        logger.info("Cancelling order", { order_id: orderId, refund_channel: args.refund_channel });
